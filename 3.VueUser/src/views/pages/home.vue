	...
	<div>
		<div class="home_box">
			...
			<div class="homeList_view">
				<div class="homeList_title">候鸟数据展示</div>
				<div class="home_list_five">
					<div v-for="(item,index) in houniaoshujuHomeList" :class="'home_list_five_item' + (Number(index) + 1)">
						<div class="img_box">
							<img class="home_img" v-if="isHttp(item.houniaotupian)" :src="item.houniaotupian.split(',')[0]" alt="">
							<img class="home_img" v-else :src="item.houniaotupian?$config.url + item.houniaotupian.split(',')[0]:''" alt="">
						</div>
						<div class="homeList_content">
							<div class="homeList_title1">
								{{item.houniaomingcheng}}
							</div>
						</div>
					</div>
				</div>
				<div class="homeList_more_view" @click="moreClick('houniaoshuju')">
			...
			<!-- 迁徙路线首页展示 -->
			<div class="homeList_view">
				<div class="homeList_title">迁徙路线展示</div>
				<div class="home_list_five">
					<div v-for="(item,index) in qianxiluxianHomeList" :class="'home_list_five_item' + (Number(index) + 1)">
						<div class="img_box">
						...
						<div class="homeList_content">
							<div class="homeList_title1">
								{{item.luxianmingcheng}}
							</div>
						</div>
					</div>
				</div>
				<div class="homeList_more_view" @click="moreClick('qianxiluxian')">
					<span class="homeList_more_text">查看更多</span>
					<el-icon><DArrowRight /></el-icon>
				</div>
			</div>
...
<script setup>
	...
	//候鸟数据首页展示
	const houniaoshujuHomeList = ref([])
	const gethouniaoshujuHomeList = () => {
		context?.$http({
			url: 'houniaoshuju/list',
			method: 'get',
			params: {
				page: 1,
				limit: 4
			...
			houniaoshujuHomeList.value = res.data.data.list
		})
	}
	//迁徙路线首页展示
	const qianxiluxianHomeList = ref([])
	const getqianxiluxianHomeList = () => {
		context?.$http({
			url: 'qianxiluxian/list',
			method: 'get',
			params: {
				page: 1,
				limit: 4
			...
			qianxiluxianHomeList.value = res.data.data.list
		})
	}
	//新闻资讯弹窗
	...
	const init = () => {
		//候鸟数据首页展示
		gethouniaoshujuHomeList()
		//迁徙路线首页展示
		getqianxiluxianHomeList()
		//新闻资讯
		getNewsList()
	}
	init()
